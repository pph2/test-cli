...
async function create(projectName, options) {
  const cwd = options.cwd || process.cwd(); //process.cwd(): 返回的是当前执行node命令时候的文件夹地址
  ...
  if(fs.existsSync(targetDir)) {
    const isCreate = await createInExistargeDir(targetDir, {
      ...options,
      isCurrent,
    });
 ...
 */
async function createInExistargeDir(targetDir, options) {
  const { force, inCurrent } = options;
  // 强制创建
  if(force && !inCurrent) {
    await fs.remove(targetDir); // 清楚当前文件和文件夹
    return true;
  }
